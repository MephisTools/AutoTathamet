...
function inject (bot) {
  ...
  bot.pickupEveryItems = () => {
    bot._client.on('D2GS_ITEMACTIONWORLD', (item) => {
      try {
        bot.say(`I should pick this item: ${bot.checkItem(item)}`)
      } catch (error) {
        console.log(error)
      }
      // bot.pickItem(item)
  ...
  bot.checkItem = (item) => {
    ...
    */
    ...
    if (item['is_weapon']) {
      itemData = diablo2Data.weapon.find(w => w['code'] === item['type'])
    }
    ...
    bot.pickit['conditions'].forEach(condition => {
      ...
      if (item['name'].toLowerCase() === condition['name'] /* && item['quality'] === condition['quality'] && item['ethereal'] === condition['ethereal'] */) {
        console.log('Name correspond')
