// TODO: TO BE ABLE TO HANDLE MULTI BOTS COMMANDS, COMMANDS SHOULD BE LIKE THAT .[bot name] [command] [args]
// EXAMPLE: .sorcFire rushb 0.5 3 true
// AND WE COULD IMAGINE SOME GROUPED COMMAND LIKE
// .all [command] [args]
// .defensive [command] [args]
// .tank [command] [args]
// .caster [command] [args]
// .melee [command] [args]
// ...
  ...
  bot._client.on('D2GS_GAMECHAT', ({ charName, message }) => {
    ...
    if (bot.master !== null) { // Just a security
      ...
      if (message.startsWith('.item') && charName === bot.master.name && message.split(' ').length > 1) {
        bot.say(`Found ${bot.itemByName(message.split(' ')[1], [diablo2Data.itemEnums.ContainerType.unspecified, diablo2Data.itemEnums.ContainerType.inventory,
          diablo2Data.itemEnums.ContainerType.inventory_bottom])}`)
      }
      ...
      if (message.startsWith('.do') && charName === bot.master.name && message.split(' ').length > 1) {
        switch (message.split(' ')[1]) {
          ...
          case '13':
            ...
            console.log(bot.itemByType('tbk', [diablo2Data.itemEnums.ContainerType.inventory, diablo2Data.itemEnums.ContainerType.inventory_bottom]))
            break
          case '14':
            bot.sayInSid('/chat on') // Turn on global chat
            break
        }
      }
