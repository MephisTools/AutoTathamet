...
function inject (bot) {
  ...
  bot._client.on('D2GS_GAMECHAT', ({ charName, message }) => {
    ...
    if (bot.master !== null) { // Just a security
      if ((message === '.follow' || message === '.followpf') && charName === bot.master.name) {
      ...
      if (message.startsWith('.npc') && charName === bot.master.name && message.split(' ').length > 1) {
        bot.say(`bot.area ${bot.area}`)
        if (bot.area === undefined) {
          return
        }
        bot.tasks.forEach(task => {
          if (task !== undefined) {
            bot.say(`task ${bot.tasks.findIndex(t => t !== undefined && t.heal === task.heal)},${JSON.stringify(task)}`)
          }
        })
        switch (message.split(' ')[1]) {
          case 'heal':
