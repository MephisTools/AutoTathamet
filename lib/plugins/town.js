...
function inject (bot) {
    ...
    switch (areaName) {
      case 'LEVEL_ROGUE_ENCAMPMENT':
        if (action === NpcActionEnum.heal) {
          id = diablo2Data.npcsByName['Akara'].id
        }
        if (action === NpcActionEnum.repair) {
          id = diablo2Data.npcsByName['Charsi'].id
        }
        break
      case 'LEVEL_LUT_GHOLEIN':
        if (action === NpcActionEnum.heal || action === NpcActionEnum.repair) {
          id = diablo2Data.npcsByName['Fara'].id
        }
        break
      case 'LEVEL_KURAST_DOCKTOWN':
        if (action === NpcActionEnum.heal) {
          id = diablo2Data.npcsByName['Ormus'].id
        }
        if (action === NpcActionEnum.repair) {
          id = diablo2Data.npcsByName['Hratli'].id
        }
        break
      case 'LEVEL_THE_PANDEMONIUM_FORTRESS':
        if (action === NpcActionEnum.heal) {
          id = diablo2Data.npcsByName['Jamella'].id
        }
        if (action === NpcActionEnum.repair) {
          id = diablo2Data.npcsByName['Halbu'].id
        }
        break
      case 'LEVEL_HARROGATH':
        if (action === NpcActionEnum.heal) {
          id = diablo2Data.npcsByName['Malah'].id
        }
        if (action === NpcActionEnum.repair) {
          id = diablo2Data.npcsByName['Larzuk'].id
        }
        break
    }
    return id
  ...
  bot.reachNpc = (action) => {
    try {
      const areaName = bot.areasById[bot.area].name
      bot.say(`bot.reachNpc areaName ${areaName}`)
      const npc = bot.npcs.find(npc => npc['unitCode'] === bot.whichNpc(action, areaName))
      bot.say(`bot.reachNpc npc ${npc}`)

      // Store the list of items of the npc
      bot._client.on('D2GS_ITEMACTIONWORLD', (callbackShop))

      bot.moveToEntity(false, npc['id'])
      // bot.tradeNpc(npc.id)
      return npc['id']
    } catch (error) {
