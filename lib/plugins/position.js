  ...
  */
  ...
  async function movementWithMapFilling (teleportation, destX, destY) {
    ...
    if (Math.abs(previousPosition.x - bot.x) < 2 && Math.abs(previousPosition.y - bot.y) < 2) { // If the bot is stuck
      bot.wss.broadcast(JSON.stringify({ protocol: 'event', name: 'obstacle', params: dest }))
      bot.say(`Obstacle at ${dest.x.toFixed(2)} - ${dest.y.toFixed(2)}`)
      bot.map.setAtPosition(dest, true)
      return false
    } else {
      bot.wss.broadcast(JSON.stringify({ protocol: 'event', name: 'clear', params: { x: dest.x, y: dest.y } }))
