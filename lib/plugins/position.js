  ...
  */
  ...
  async function reachedWarp (direction = DirectionsEnum.left) {
    ...
    while (!reachedCorner) {
      ...
      bot.moveTo(true, nextPos.x, nextPos.y)
  ...
  async function pf (teleportation, x, y) {
    ...
    // TODO: FIX TIMEOUT THING
    while (Math.sqrt((bot.x - x) * (bot.x - x) + (bot.y - y) * (bot.y - y)) > 25.0/* || +new Date() < start + 20000 */) { // timeout
      bot.say(`Calculated distance ${Math.sqrt((bot.x - x) * (bot.x - x) + (bot.y - y) * (bot.y - y)).toFixed(2)}`)
      bot.say(`Am i arrived ? ${Math.sqrt((bot.x - x) * (bot.x - x) + (bot.y - y) * (bot.y - y)) > 25.0}`)
