  ...
  async function movementWithMapFilling (teleportation, destX, destY) {
    ...
    if (Math.abs(previousPosition.x - currentPosition.x) < 2 && Math.abs(previousPosition.y - currentPosition.y) < 2) { // If the bot is stuck
      ...
      if (teleportation) {
        ...
        const distance = Utils.distance(dest, currentPosition)
        const dY = (destY - currentPosition.y) / distance
        const dX = (destX - currentPosition.x) / distance
        obstacle = { x: currentPosition.x + dX * 5, y: currentPosition.y + dY * 5 }
      }
      bot.say('distance is ' + Utils.distance(bot, obstacle))
