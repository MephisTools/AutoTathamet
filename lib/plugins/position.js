  ...
  */
  ...
  bot.findWarp = async (teleportation) => {
    ...
    while (true) {
      ...
      console.log(pos)
      const r = await Promise.race([bot.moveTo(teleportation, pos.x, pos.y), Utils.delay(60000)])
      if (done || r === false) {
        return
      }
    }
  ...
  bot.moveToEntity = async (teleportation, entityId) => {
    let entity = bot.npcs.find(npc => { return npc['unitId'] === entityId })
    let type = 1
    if (entity === undefined) {
  ...
  async function pf2Internal (teleportation, x, y) {
    ...
    while (Utils.distance({ x: bot.x, y: bot.y }, bot.destination) > 10.0) {
      const distance = Utils.distance({ x: bot.x, y: bot.y }, bot.destination)
      verboseSay(`Calculated distance ${distance.toFixed(2)}`)
      verboseSay(`Am i arrived ? ${distance <= 10.0}`)
  ...
  bot.base = async () => {
    return new Promise(resolve => {
      ...
      const callback = ({ ownerId, ownerName, localId, remoteId }) => {
        bot._client.write('D2GS_RUNTOENTITY', { // Go to the portal
          entityType: 2,
          entityId: localId
        })
      ...
      if (bot.hasItem('tbk')) { // If we have a tome of portal
