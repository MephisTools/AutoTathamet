function inject (bot) {
  ...
  // Check if enough life / mana, take or not potion
  bot.checkLifeMana = () => {
    if (bot.life < bot.maxLife / 2) { // Treshold ?
      if (bot.usePotion('hp')) {
        bot.say('I used hp potion')
      }
    }
    if (bot.mana < bot.maxMana / 2) {
      if (bot.usePotion('mp')) {
  ...
  bot.usePotion = (type) => {
    ...
    const pot = bot.inventory.find(item => { return item['type'].includes(type) }) // Includes = any hp / mp
    if (pot === undefined) {
      return false
    }
